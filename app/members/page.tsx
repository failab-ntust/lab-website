...
const Master = [
    ...
    { grade: 'Master', name: '吳竣宇', class: '人工智慧所', year: '112', imgSrc: '/112/吳竣宇', co_advisor: '羅乃維院長', researchfield: '討論中…', email: 'M11252003@mail.ntust.edu.tw' },
...
export default function Members() {
    ...
    const [selectedClass, setSelectedClass] = React.useState(''); // 用來儲存選擇的class

    const handleClassFilter = (className: string) => {
        setSelectedClass(className === selectedClass ? '' : className);
    }

    // 篩選符合 selectedClass 的資料
    const filteredData = selectedClass
        ? Master.filter(person => person.class === selectedClass)
        : Master;

    // 提取所有 unique class 以供選擇
